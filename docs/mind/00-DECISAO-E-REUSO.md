...
## 6. Padrão de painel Heeca (21/09/2026)

Aplicado o padrão visual da plataforma (`../../docs/PADRAO-PAINEL.md`, kit em `../../ui/app/`) em todas as páginas: a sálvia provisória (decisão D7) saiu; agora é canvas `#f5f5f7`, Inter, cartões 14 px sem sombra e **uma cor de ação — o laranja da família saúde (`#f06511`)**; o botão quase-preto (`--action`) do layout "clean" do portal deixou de valer no painel (PADRAO §6) e `--color-action` passou a apontar para a cor da família. Os tokens antigos (`bg`, `surface`, `border`, `text`, `primary`, `action`…) continuam existindo no `globals.css` apontando para o padrão, então as telas existentes seguiram funcionando; código novo usa `brand-*`, `canvas`, `ink`, `mut`, `line` e `card card-hd card-title tile tag-* btn-primary link-mut`. Casca em `components/dashboard/shell.tsx` (sidebar 232 px com ícone do produto e cartão do plano Solo → Clínica → Clínica Plus; barra superior com busca de paciente, seletor de profissional da clínica, data, sino de sessões aguardando e link público; aviso de acesso do portal logo abaixo; no celular barra inferior Início/Agenda/Pacientes/Mensagens + "Mais") — `components/layout/sidebar.tsx` e `mobile-nav.tsx` removidos. Painel inicial reescrito na ordem do padrão mantendo os indicadores do §39: sessões hoje, recebido no mês (ou realizadas, para quem não vê valores), pacientes ativos e aguardando ação; recebido por dia (+ previsto e a receber) e rosca online × presencial; comparecimento, inativos há 90+ dias e lista de espera; sessões de hoje e feed de mensagens; coluna direita com meta do mês (`Organization.metaMensalCents`, formulário em Configurações › Clínica), próxima sessão com link online, ocupação da semana em anel e ações rápidas. Escopo e permissões financeiras iguais aos de antes.
